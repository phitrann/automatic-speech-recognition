# https://github.com/EgorLakomkin/KTSpeechCrawler/blob/master/crawler/youtube_helpers.py
import hashlib
import json
import os
import copy
...
if not os.path.exists(cfg.AUDIO_PROCESSED_PATH) or not os.path.exists(
    ...
    os.makedirs(cfg.AUDIO_PROCESSED_PATH)
    os.makedirs(cfg.SUBTITLE_PROCESSED_PATH)
...
def timedelta_dt(t1, t2):
    dt1 = datetime.timedelta(
        hours=t1.hour, minutes=t1.minute, seconds=t1.second, microseconds=t1.microsecond
    )
    dt2 = datetime.timedelta(
        hours=t2.hour, minutes=t2.minute, seconds=t2.second, microseconds=t2.microsecond
    )
    return dt2 - dt1
    ...
    for s_idx in range(len(subs)):
        ...
        if s_idx == 0:
            ...
            if distance_sec < min_dist and merged_dist.total_seconds() < max_dist:
                ...
                new_s["original_phrase"] = (
                    prev_s["original_phrase"] + " " + s["original_phrase"]
                )
...
def parse_subtitle(
    ...
    all_subtitles = [
        s
        for s in all_subtitles
        if re.match(everything_cool, s["phrase"])
    ...
    all_subtitles = list(
        filter(
            lambda x: x["duration"] <= max_duration and x["duration"] >= min_duration,
            all_subtitles,
        )
    )
...
def get_closest_captions(src_caption, lst_dest_captions):
    ...
    for dest_caption in lst_dest_captions:
        dest_start_ts, dest_end_ts = (
            dest_caption.start_in_seconds,
            dest_caption.end_in_seconds,
        )
